...
test("помечает карту левосторонней secondary branch, чтобы скрыть декоративную вертикаль у подзадач", () => {
...
test("вторичные ветки могут делить одну вертикаль без горизонтального разъезда", () => {
    ...
    assert.equal(firstGeometry.trunkX, secondGeometry.trunkX);
});
...
test("вторичная ветка не уезжает по горизонтали за пределы навигационной панели", () => {
    ...
    assert.ok(geometry.trunkX >= geometry.start.x);
    assert.ok(geometry.trunkX <= 280);
    assert.ok(geometry.targets.every((target) => target.x <= 280));
});

test("сохраняет координату вертикали, если прежняя позиция всё ещё валидна", () => {
    const initialGeometry = buildAnchoredConnectionGeometry({
        rootRect: createRect(0, 0, 320, 640),
        anchorRect: createRect(24, 120, 220, 52),
        targetRects: [
            createRect(40, 184, 204, 46),
    ...
    const updatedGeometry = buildAnchoredConnectionGeometry({
        rootRect: createRect(0, 0, 320, 640),
        anchorRect: createRect(24, 120, 208, 52),
        targetRects: [
            createRect(40, 184, 192, 46),
            createRect(40, 236, 192, 46)
        ],
        side: "right",
        preferredTrunkX: initialGeometry.trunkX
    });

    assert.equal(updatedGeometry.trunkX, initialGeometry.trunkX);
    assert.ok(updatedGeometry.trunkX > updatedGeometry.start.x);
    assert.ok(updatedGeometry.targets.every((target) => target.x < updatedGeometry.trunkX));
});
...
test("активный дочерний блок перекрашивается только после достижения его анимацией", () => {
    const dom = new JSDOM(createOverlayFixture());
    const restoreGlobals = installDomGlobals(dom.window);
    ...
    try {
        ...
        scenarioButton.classList.add("flow-block", "flow-block--toggle", "flow-block--active");
        childBlock.classList.add("flow-block", "flow-block--subtask", "flow-block--active");
        ...
        assert.equal(
            childBlock.dataset.flowBlockActiveTag ?? null,
            null,
            "Дочерний блок не должен перекрашиваться до завершения его ветки"
        );

        advanceAnimationFrames(4);
        assert.equal(
            scenarioButton.dataset.flowBlockActiveTag,
            "branching",
            "Родительский блок должен перекраситься, когда основная линия дошла до него"
        );
        assert.equal(childBlock.dataset.flowBlockActiveTag ?? null, null);

        advanceAnimationFrames(12);
        assert.equal(
            childBlock.dataset.flowBlockActiveTag,
            "branching",
            "Дочерний блок должен перекрашиваться только после дорисовки secondary branch до него"
        );
    } finally {
        restoreGlobals();
        dom.window.close();
...
test("не сдвигает вертикаль вторичной ветки после смены размеров блока", () => {
    const dom = new JSDOM(createOverlayFixture());
    const restoreGlobals = installDomGlobals(dom.window);
    ...
    try {
        ...
        const initialBranchPath = canvas.querySelector(".tag-connection-map__path--branch");
        const initialPoints = parsePathPoints(initialBranchPath?.getAttribute("d") ?? "");
        const initialTrunkX = initialPoints[1]?.x;

        assignRect(scenarioButton, createRect(24, 140, 208, 52));
        assignRect(childBlock, createRect(40, 204, 192, 46));

        redrawNavigationTagConnections(appRoot, { preserveAnimation: true });
        stepRafQueue();
        advanceAnimationFrames(20);

        const updatedBranchPath = canvas.querySelector(".tag-connection-map__path--branch");
        const updatedPoints = parsePathPoints(updatedBranchPath?.getAttribute("d") ?? "");
        const updatedTrunkX = updatedPoints[1]?.x;

        assert.equal(updatedTrunkX, initialTrunkX);
    } finally {
        restoreGlobals();
        dom.window.close();
    }
...
test("быстрое переключение между тегами не показывает вторичную ветку сразу готовой", () => {
    const dom = new JSDOM(
        createOverlayFixture()
            .replace('data-tags="branching"', 'data-tags="branching navigation"')
            .replace('data-tag-connection-target="branching"', 'data-tag-connection-target="branching navigation"')
    );
    const restoreGlobals = installDomGlobals(dom.window);
    ...
    try {
        ...
        const navigationLane = appRoot.querySelector(".lesson-lane--navigation");
        const canvas = appRoot.querySelector("[data-tag-connection-canvas]");
        const branchingButton = appRoot.querySelector('[data-tag-legend-control="branching"]');
        const scenarioButton = appRoot.querySelector('[data-scenario-toggle="branch-safety"]');
        const childBlock = appRoot.querySelector("[data-tag-branch-target]");

        const navigationButton = dom.window.document.createElement("button");
        navigationButton.type = "button";
        navigationButton.dataset.tagLegendControl = "navigation";
        navigationButton.textContent = "Navigation";
        mapRoot.querySelector(".scenario-legend")?.append(navigationButton);

        assignRect(layoutRoot, createRect(0, 0, 960, 640));
        assignRect(navigationBody, createRect(0, 40, 280, 240));
        assignRect(mapRoot, createRect(0, 20, 280, 520));
        assignRect(branchingButton, createRect(20, 60, 96, 28));
        assignRect(navigationButton, createRect(132, 60, 104, 28));
        assignRect(scenarioButton, createRect(24, 140, 220, 52));
        assignRect(childBlock, createRect(40, 204, 204, 46));
        ...
        const completePathData = canvas.querySelector(".tag-connection-map__path--branch")?.getAttribute("d") ?? "";
        assert.ok(completePathData.length > 0, "Первая ветка должна успеть полностью отрисоваться");

        navigationLane.dataset.highlightTag = "navigation";
        redrawNavigationTagConnections(appRoot);
        stepRafQueue();

        const switchedBranchPath = canvas.querySelector(".tag-connection-map__path--branch");
        const switchedInitialPathData = switchedBranchPath?.getAttribute("d") ?? "";
        assert.ok(switchedBranchPath, "После переключения ветка должна остаться в DOM");
        assert.ok(
            switchedInitialPathData.length < completePathData.length,
            "После быстрого переключения тега ветка должна стартовать заново, а не появляться полностью готовой"
        );
        assert.ok(
            (switchedBranchPath?.__branchStateData?.currentVisibleLength ?? 0)
                < (switchedBranchPath?.__branchStateData?.pathLength ?? 0),
            "После переключения новая ветка должна находиться в процессе дорисовки"
        );

        advanceAnimationFrames(20);
        const switchedCompletedBranchPath = canvas.querySelector(".tag-connection-map__path--branch");
        const switchedCompletedPathData = switchedCompletedBranchPath?.getAttribute("d") ?? "";
        assert.ok(
            switchedCompletedPathData.length > switchedInitialPathData.length,
            "После завершения анимации ветка должна дорисоваться до полной длины"
        );
        assert.equal(
            switchedCompletedBranchPath?.__branchStateData?.currentVisibleLength,
            switchedCompletedBranchPath?.__branchStateData?.pathLength
        );
    } finally {
        restoreGlobals();
        dom.window.close();
...
test("после быстрого скрытия и повторного показа панели вторичная ветка заново дорисовывается", () => {
    const dom = new JSDOM(createOverlayFixture());
    const restoreGlobals = installDomGlobals(dom.window);
    ...
    try {
        ...
        const completedPathData = canvas.querySelector(".tag-connection-map__path--branch")?.getAttribute("d") ?? "";
        assert.ok(completedPathData.length > 0, "Ветка должна успеть полностью отрисоваться до скрытия панели");

        layoutRoot.classList.add("lesson-layout--navigation-transitioning");
        redrawNavigationTagConnections(appRoot);
        stepRafQueue();
        ...
        const branchPathAfterReshow = canvas.querySelector(".tag-connection-map__path--branch");
        const restartPathData = branchPathAfterReshow?.getAttribute("d") ?? "";
        assert.ok(branchPathAfterReshow, "После повторного показа ветка должна снова появиться в DOM");
        assert.ok(
            restartPathData.length < completedPathData.length,
            "После быстрого повторного показа secondary branch не должна появляться сразу полностью готовой"
        );
        assert.ok(
            (branchPathAfterReshow?.__branchStateData?.currentVisibleLength ?? 0)
                < (branchPathAfterReshow?.__branchStateData?.pathLength ?? 0),
            "После повторного показа ветка должна находиться в процессе дорисовки"
        );
    } finally {
        restoreGlobals();
        dom.window.close();
