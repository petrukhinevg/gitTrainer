...
2. Исполнитель проверяет, что задача имеет ясный результат, корректные labels и при необходимости активный parent issue.
...
4. Исполнитель берёт задачу в работу в её выделенной ветке и держит scope задачи пригодным для review.
5. После реализации исполнитель пушит ветку, создаёт или обновляет PR и затем переводит задачу в `Review` по правилам из `docs/agent/GIT_WORKFLOW.md` и `docs/agent/BOARD_WORKFLOW.md`.
6. Если review находит проблемы, исправления вносятся в той же task branch отдельным follow-up commit.
...
- Если задача меняет правила работы репозитория, обновляй один профильный документ вместо копирования правила в несколько файлов.
- Если в ходе задачи обнаружилась стабильная операционная заметка по `gh` или GraphQL, переноси её в `docs/agent/GITHUB_AUTOMATION.md`, пока детали свежи.
