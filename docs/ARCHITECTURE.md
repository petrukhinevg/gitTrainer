...
- Корневой проект: единый репозиторий с backend на Spring Boot и отдельным SPA-клиентом; backend может отдавать собранную SPA в production-сборке
- `frontend/`: SPA-клиент тренировочного рабочего пространства, который развивается как отдельная часть того же репозитория
