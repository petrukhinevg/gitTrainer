...
- Для `gh` используй переменную `GH_TOKEN`; если токен загружается из `.env`, предпочитай `set -a && source .env && set +a`, чтобы переменные точно экспортировались в окружение команды.
...
- Новые child issues добавляй только в действующие эпики; если parent issue уже в `Done` на GitHub board, не расширяй его новыми задачами и сначала создай новый активный parent issue.
...
- Замечания по naming, структуре и читаемости тоже проверяй, но выноси их в findings только когда они создают реальный риск сопровождения, маскируют поведение, ломают принятые границы или повышают вероятность ошибки.
